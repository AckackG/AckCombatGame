...
- 怪物专用武器使用 `monster_only` 标记，只能显式获取，不能进入普通/特殊随机武器池。
- **距离限制**: 设定两级距离机制。武器 `range` 决定单位理想交战距离，`Range_Max` (默认 3.5 倍 range) 决定子弹强制消失的极限距离。
- **伤害衰减**: 子弹飞行距离超过衰减起点后伤害开始线性衰减，直到 `Range_Max` 时降至最低 25%。
